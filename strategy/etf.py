#coding:gbk
# 稳健版 ETF 趋势轮动策略 v4.7（与 miniqmt_etf_strategy 逻辑对齐）
import numpy as np
import pandas as pd
import datetime
import json
...
def get_current_price(C, stock):
    """获取标的实时价格：优先 get_instrument_detail（昨收），其次日线收盘价降级。"""
    # 1. 尝试 get_instrument_detail 取昨收（最可靠，回测/模拟盘都有）
    try:
        ...
        if isinstance(detail, tuple) and len(detail) >= 1:
            ...
            if isinstance(d, dict):
                pre_close = float(d.get('PreClose', 0) or d.get('preClose', 0))
            elif hasattr(d, 'PreClose'):
                pre_close = float(getattr(d, 'PreClose', 0))
        elif isinstance(detail, dict):
            pre_close = float(detail.get('PreClose', 0) or detail.get('preClose', 0))
        elif hasattr(detail, 'PreClose'):
            pre_close = float(getattr(detail, 'PreClose', 0))
        if pre_close > 0 and pre_close < 1e6:
            return pre_close
    except Exception:
        pass
    ...
    try:
        ...
        if data and stock in data:
            v = get_latest_valid_price(data[stock])
            if v > 0:
                return v
    except Exception as e:
        print('[行情] get_market_data_ex 失败 %s: %s' % (stock, e))
...
def safe_order(C, side, etf, volume, remark):
    """执行交易指令并登记到订单簿中（增加 passorder 返回值诊断，对齐 miniqmt 版本）"""
    etf = normalize_stock_code(etf)
    ...
    try:
        ...
        print('[下单诊断] passorder(%s,%s,%s) => order_id=%r (type=%s)' % (side, etf, volume, order_id, type(order_id).__name__))
        ...
        # 模拟盘 passorder 返回 0 是正常的（QMT 模拟盘特性）
        # 仍然按已提交处理，登记到 order_book 中
        if str(order_id) == '0':
            print('[下单诊断] passorder 返回 0（模拟盘正常行为），按已提交处理: %s %s %d 股' % (side, etf, volume))
            C.order_book[str(order_id)] = {
                "etf": etf,
                "side": side,
                "volume": volume,
        ...
        print('[下单失败] 未返回有效订单号:', etf, side, volume, remark, 'returned:', order_id)
        return None
    except Exception as e:
        print("[下单失败]", e)
        return None
...
def execute_trades(C, current_date):
    ...
    try:
        ...
        # 同步订单状态，然后重新获取实时可用资金（与 miniqmt 版本对齐）
        sync_order_book(C)
...
def execute_pending_orders(C, current_date):
    """每日开盘尝试处理遗留未成交补单（与 miniqmt 版本逻辑对齐）"""
    ...
    for etf, order in list(C.pending_orders.items()):
        ...
        need_shares = int(order.get("shares", 0) / 100) * 100
        if need_shares <= 0:
            expired.append(etf)
            continue
        ...
        # 取现金能买的最大股数与实际需要的较小值（与 miniqmt 版本对齐）
        max_shares = int(available_cash * 0.98 / price / 100) * 100
        buy_shares = min(need_shares, max_shares)
        ...
        remark = '补单完成' if buy_shares == need_shares else '补单部分'
        print('[%s] [补单] %s %s %d 股' % (current_date, remark, etf, buy_shares))
        order_id = safe_order(C, 'buy', etf, buy_shares, remark)
        if order_id:
            available_cash -= buy_shares * price * 1.02
            order["last_order_id"] = str(order_id)
            order["shares"] = need_shares - buy_shares
            if order["shares"] < 100:
                expired.append(etf)
...
def trigger_lockdown(C, current_date):
    """触发账户全局空仓保护（对齐 miniqmt 版本：锁仓前先撤活跃买单）"""
    C.in_lockdown = True
    C.lockdown_days_left = C.lockdown_length
    C.pending_orders = {}
    # 锁仓前撤活跃买单，防止锁仓后买单继续成交
    cancel_active_buy_orders(C, current_date)
    submit_lockdown_liquidation(C, current_date)


def cancel_active_buy_orders(C, current_date):
    """锁仓前撤销所有活跃买单"""
    canceled = 0
    for order_id, order in list(getattr(C, 'order_book', {}).items()):
        ...
        if request_cancel_order(C, order_id, order):
            canceled += 1
    if canceled > 0:
        print('[%s] [风控] 锁仓前撤销活跃买单: %s 笔' % (current_date, canceled))
...
def handlebar(C):
    ...
    # 非回测/模拟盘：每次 handlebar 调用都同步订单并归档终态
    if not is_backtest:
        cancel_stale_orders(C, current_date)
